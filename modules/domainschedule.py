...
talentascension = json.load(open("attributes/Guide/talent_ascension_config.json", "r"))
weaponascension = json.load(open("attributes/Guide/weapon_ascension_config.json", "r"))
...
class DomainSchedule(commands.Cog):
    ...
    async def update_domains(self):
        ...
        talents_channel = self.bot.get_channel(int(json.load(open('config.json', 'r'))['settings']['talent-schedule-channel-id']))
        weapons_channel = self.bot.get_channel(int(json.load(open('config.json', 'r'))['settings']['weapon-schedule-channel-id']))
                    ...
                    for day in talent['days']:
                        if today == day:
                            if today == "Sunday":
                                ...
                                desc+= "\n"
            domain_img = f"attributes/Img/TalentDomains/{today}.png"
    ...
    async def weapons(self, msg: discord.Message): # Weapon ascension domain schedule
        ...
        for region in regions:
            ...
            domain_img = f"attributes/Img/WeaponDomains/{today}.png"
            embed.set_image(url = f"attachment://{today}.png")
            embed.add_field(name=f"{today}'s Schedule", value=desc, inline=True)
